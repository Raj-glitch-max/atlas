...
Atlas reuses the governance already present in `CONTRIBUTING.md`, `.github/`, `Makefile`, `scripts/frozen-docs.list`, and `agents/agents/GOVERNANCE.md`. This file summarizes those active rules without adding product or architecture policy.
...
1. Record a journal entry at `agents/agents/journal/<YYYY-MM-DD>-<slug>.md`.
...
- Decisions, dissent, overrides, and required frozen-doc changes are recorded under `agents/agents/journal/`.
- Preserve disagreement instead of smoothing it into consensus.
- Founder overrides are allowed, but must be recorded with the reasoning and reconsideration conditions described in `agents/agents/GOVERNANCE.md`.
